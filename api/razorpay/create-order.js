...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    const { data: order, error: orderError } = await supabase.from('orders').select('id,order_number,total,status,user_id').eq('id', orderId).eq('user_id', user.id).single();
    ...
    const { error: paymentError } = await supabase.from('payments').upsert({ order_id: order.id, user_id: user.id, provider: 'razorpay', razorpay_order_id: rzpOrder.id, amount: Number(order.total), status: 'created' }, { onConflict: 'order_id' });
