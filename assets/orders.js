...
(function () {
  ...
  // Fetch each part independently. Keep the item/address selects limited to
  // columns used by the page so an optional/missing column cannot break View Order.
  async function getOrderByNumber(n){
    ...
    const addressPromise=order.address_id
      ?window.agriSupabase.from('addresses').select('id,name,phone,line,village,city,district,state,pin').eq('id',order.address_id).eq('user_id',user.id).maybeSingle()
      :Promise.resolve({data:null,error:null});
    // Do not require line_total; the UI calculates it from unit_price × quantity.
    const itemsPromise=window.agriSupabase.from('order_items').select('id,product_id,product_name,unit_price,quantity').eq('order_id',order.id);
    ...
    const orderItems=(itemsResult.data||[]).map(i=>({...i,line_total:Number(i.unit_price||0)*Number(i.quantity||0)}));
    return {...order,addresses:addressResult.data||null,order_items:orderItems,payments:paymentResult.error?[]:(paymentResult.data||[])};
  }
