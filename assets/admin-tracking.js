...
(function(){
  ...
  function render(){
    ...
    if(box)box.innerHTML=html;else document.getElementById('adminContent').insertAdjacentHTML('afterbegin','<div id="adminTrackingPanel"></div>'+html);
    document.querySelectorAll('[data-track-save]').forEach(btn=>btn.addEventListener('click',()=>{const id=btn.getAttribute('data-track-save');const sel=document.querySelector('[data-track-select="'+id+'"]');saveStatus(id,sel.value);}));
  }
  function init(){ready=true;setTimeout(render,250);setInterval(()=>{if(document.getElementById('adminContent')&&typeof db!=='undefined'&&db.orders)render();},15000);}
