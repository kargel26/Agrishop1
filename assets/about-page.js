...
(function(){
  ...
  function render(p){
    ...
    if(p.is_active===false){main.innerHTML='<div class="container"><section class="pad"><div class="panel-card" style="max-width:760px;margin:40px auto;text-align:center"><h2>About AgriMart</h2><p class="sub">This page is temporarily unavailable.</p></div></section></div>';return;}
    const stats=Array.isArray(p.stats)?p.stats.filter(x=>x&&String(x.value??'').trim()&&String(x.label??'').trim()):[];
    main.innerHTML=`${p.hero_visible!==false?`<div class="hero" style="padding:0"><div class="container hero-inner" style="grid-template-columns:1fr;padding:56px 0"><div style="max-width:640px"><span class="hero-eyebrow">${esc(p.hero_eyebrow)}</span><h1 style="font-size:36px">${esc(p.hero_title)}</h1><p class="sub">${esc(p.hero_description)}</p></div></div></div>`:''}${p.stats_visible!==false?`<section class="pad"><div class="container"><div class="stat-grid">${stats.map(s=>`<div class="stat-card center"><div class="val">${esc(s.value)}</div><div class="lbl">${esc(s.label)}</div></div>`).join('')}</div></div></section>`:''}${p.mission_visible!==false?`<section class="pad bg-alt"><div class="container"><div class="section-head"><div><span class="eyebrow">${esc(p.mission_eyebrow)}</span><h2>${esc(p.mission_title)}</h2><p>${esc(p.mission_text)}</p></div></div></div></section>`:''}`;
  }
  async function load(){
    render(DEFAULT);
    try{
      ...
      render(data||{...DEFAULT,is_active:false});
