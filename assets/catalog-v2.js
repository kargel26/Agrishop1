...
(function(){
  ...
  window.AgriCatalog={ready:(async()=>{try{await loadProducts();const u=await user();if(u)await syncCart(u.id);}catch(e){console.error('AgriMart Supabase catalog:',e);/* Keep the legacy catalog available so Wishlist/Shop do not become blank when Supabase is unavailable. */window.AGRI_PRODUCTS=(typeof PRODUCTS!=='undefined'?PRODUCTS.slice():[]);}return window.AGRI_PRODUCTS||[];})()};
